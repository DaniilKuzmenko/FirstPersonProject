...
public class Slot : MonoBehaviour, IDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler, IPointerEnterHandler, IPointerExitHandler
{
        ...
        else if (!itemData.isUniq)
        {
            for (int i = 0; i < slots.Count; i++)
            {
                if (slots[i].GetComponent<Slot>().itemData.id == itemData.id)
                {
                    ...
                    slots[i].transform.Find("ItemCount").GetComponent<Text>().text = slots[i].GetComponent<Slot>().itemData.count.ToString();
                    Destroy(gameObject);
                    break;
                }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        float slotInventoryContentDistance = Vector3.Distance(transform.position, InventoryManager.instance.GetInventoryContent().transform.position);
        float slotChestContentDistance = Vector3.Distance(transform.position, InventoryManager.instance.GetChestContent().transform.position);
